...
export function createOrderEmailHtml(id: number, order: Order, menu: DayMenu[]) {
  ...
  <body style="font-family:Arial,sans-serif;font-size:12pt">
    ...
    <div style="margin:0 0 1em 0">
      ...
      <ul>
        <li><b>Имя:</b> ${order.customer.fullName}</li>
        <li><b>Компания:</b> ${order.customer.company}</li>
        <li><b>Офис:</b> ${order.customer.office}</li>
        <li><b>Этаж:</b> ${order.customer.floor}</li>
        <li><b>Телефон:</b> ${order.customer.phone}</li>
      </ul>
    </div>
    <div style="margin:0 0 1em 0">
    ...
    <div>
      ...
      <ul>
        ...
        <li><b>Итоговая сумма:</b> ${order.total} тнг</li>
      </ul>
    </div>
  </body>
  </html>`;
